...
contract WalletFactory {
	...
	// When the relayer needs to act upon an /identity/:addr/submit call, it'll either call execute on the Wallet directly
	...
	function deployAndExecute(
		...
		Wallet.Transaction[] calldata txns, bytes calldata signature
	) external {
		address payable addr = payable(deploySafe(code, salt));
		Wallet(addr).execute(txns, signature);
	}
