...
contract QuickAccManager {
	...
	event LogScheduled(bytes32 indexed txnHash, bytes32 indexed accHash, address indexed signer, uint nonce, uint time, Wallet.Transaction[] txns);
	...
	// WARNING: if the signature of this is changed, we have to change WalletFactory
	function send(Wallet identity, QuickAccount calldata acc, DualSig calldata sigs, Wallet.Transaction[] calldata txns) external {
	...
	function cancel(Wallet identity, QuickAccount calldata acc, uint nonce, bytes calldata sig, Wallet.Transaction[] calldata txns) external {
		bytes32 accHash = keccak256(abi.encode(acc));
		require(identity.privileges(address(this)) == accHash, 'WRONG_ACC_OR_NO_PRIV');
	...
	function execScheduled(Wallet identity, bytes32 accHash, uint nonce, Wallet.Transaction[] calldata txns) external {
		require(identity.privileges(address(this)) == accHash, 'WRONG_ACC_OR_NO_PRIV');
	...
	function isValidSignature(bytes32 hash, bytes calldata signature) external view returns (bytes4) {
		...
		if (Wallet(payable(address(msg.sender))).privileges(address(this)) == accHash) {
	...
	// WARNING: if the signature of this is changed, we have to change WalletFactory
	function sendTransfer(Wallet identity, QuickAccount calldata acc, DualSigAlwaysBoth calldata sigs, Transfer calldata t) external {
		...
		require(acc.one == SignatureValidator.recoverAddr(hash, sigs.one), 'SIG_ONE');
		require(acc.two == SignatureValidator.recoverAddr(hash, sigs.two), 'SIG_TWO');
		Wallet.Transaction[] memory txns = new Wallet.Transaction[](2);
		txns[0].to = t.token;
	...
	// WARNING: if the signature of this is changed, we have to change WalletFactory
	function sendTxns(Wallet identity, QuickAccount calldata acc, DualSigAlwaysBoth calldata sigs, Txn[] calldata txns) external {
		require(identity.privileges(address(this)) == keccak256(abi.encode(acc)), 'WRONG_ACC_OR_NO_PRIV');

		// hashing + prepping args
		bytes32[] memory txnBytes = new bytes32[](txns.length);
		Wallet.Transaction[] memory identityTxns = new Wallet.Transaction[](txns.length);
